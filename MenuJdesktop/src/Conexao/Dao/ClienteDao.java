...
public class ClienteDao {
	...
	// query para inserir no banco de dados usando o objeto cliente 
			...
			System.out.println(query); // somente um print no terminal para validação se a query foi executada corretamente
	...
	public ArrayList<Cliente> listarClientePorNome(String nome) {
		...
		ArrayList<Cliente> listaDeClientes = new ArrayList<>(); // criação de um Array para armazenar os objetos de clientes que vamos buscar no banco de dados
		...
		try {
			...
			while (conectabancodao.getResultSet().next()) { //aqui criamos uma repetição para armazenar no array cada resultado em um objeto cliente diferente para listar todos de uma vez no Scroll pane do Windows Builder
				...
				Cliente clientes = new Cliente();  // criamos um novo objeto Cliente para cada repetição para armazenarmos no Array

				clientes.setCod_cliente(conectabancodao.getResultSet().getString("cod_cliente")); //adicionamos ao objeto criado acima cada campo do resultado do banco de dados
				clientes.setNome_cliente(conectabancodao.getResultSet().getString("nome_cliente"));
				clientes.setCpf_cliente(conectabancodao.getResultSet().getString("cpf_cliente"));
				clientes.setRg_cliente(conectabancodao.getResultSet().getString("rg_cliente"));
				clientes.setEmail_cliente(conectabancodao.getResultSet().getString("email_cliente"));
				...
				listaDeClientes.add(clientes); // por fim pegamos o objeto cliente criado acima e alocamos ele dentro de um Array para listar futuramente no Scroll Pane
			}
	...
	public void deletarClientePorId(Cliente cliente) { //Optmos por receber o objeto cliente inteiro mas vamos usar somente o ID dele para realizar o delete deste cadastro no banco de dados
		try {
	...
	public void alterarClientePorId(Cliente cliente) { //Mesmo processo do Delete, porém aqui realizamos o update do cadastro de cliente usando o ID e buscamos todos os dados digitados no Windows Builder
		try {
